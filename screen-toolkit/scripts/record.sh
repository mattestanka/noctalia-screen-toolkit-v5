...
case "$ACTION" in
  ...
  stop)
    ...
    if [ "$BIN" = "gpu-screen-recorder" ]; then
        # Its process name exceeds Linux's 15-byte comm limit, so a normal
        # `pkill gpu-screen-recorder` never matches. Match the full command
        # line and send SIGINT so the recorder flushes/finalizes the video.
        pkill -INT -f '^(/.*/)?gpu-screen-recorder' 2>/dev/null \
            || pkill -INT -f 'com\.dec05eba\.gpu_screen_recorder' 2>/dev/null \
            || true
    else

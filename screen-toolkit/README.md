...
This fork is based on Alexander's independent Noctalia v5 remake/port and
continues the Mango/NVIDIA fixes from the mattestanka v4 fork.

## Fork changes

- Colour picking always uses a transparent `slurp` point selector, waits for
  the compositor to settle, and captures the exact pixel. This avoids sampling
  a software cursor or stale selector frame on NVIDIA.
- GPU Screen Recorder is preferred and MP4 is the default recording format;
...
- **`gpu-screen-recorder`** — preferred for region and fullscreen recording,
  especially on NVIDIA (NVENC)
...
```sh
noctalia msg panel-toggle mattestanka/screen-toolkit:panel
```

Open the legacy tools panel (the 4×2 grid with subpanels):

```sh
noctalia msg panel-toggle mattestanka/screen-toolkit:panel-legacy
```

Open the result panel (shows the last capture/recording output):

```sh
noctalia msg panel-toggle mattestanka/screen-toolkit:result
```
...
```sh
noctalia msg plugin mattestanka/screen-toolkit:service all toggle
noctalia msg plugin mattestanka/screen-toolkit:service all colorPicker
noctalia msg plugin mattestanka/screen-toolkit:service all ocr
noctalia msg plugin mattestanka/screen-toolkit:service all qr
noctalia msg plugin mattestanka/screen-toolkit:service all palette
noctalia msg plugin mattestanka/screen-toolkit:service all lens
noctalia msg plugin mattestanka/screen-toolkit:service all measure
...
noctalia msg plugin mattestanka/screen-toolkit:service all recordFullscreen
noctalia msg plugin mattestanka/screen-toolkit:service all recordFullscreenGif
noctalia msg plugin mattestanka/screen-toolkit:service all recordFullscreenMp4
noctalia msg plugin mattestanka/screen-toolkit:service all recordStop
noctalia msg plugin mattestanka/screen-toolkit:service all recordSave
noctalia msg plugin mattestanka/screen-toolkit:service all recordCopy
noctalia msg plugin mattestanka/screen-toolkit:service all recordDiscard
noctalia msg plugin mattestanka/screen-toolkit:service all ocrSearch
noctalia msg plugin mattestanka/screen-toolkit:service all clearResult
noctalia msg plugin mattestanka/screen-toolkit:service all clearHistory
```
...
noctalia msg plugin mattestanka/screen-toolkit:service all ocrTranslate en
# Search the current OCR text (or a payload.text) with the configured engine
noctalia msg plugin mattestanka/screen-toolkit:service all ocrSearch
# Share a file on disk (absolute path payload)
noctalia msg plugin mattestanka/screen-toolkit:service all share /path/to/image.png
# Save the finished recording (payload format: "gif" or "mp4")
noctalia msg plugin mattestanka/screen-toolkit:service all recordSave mp4
# Show or hide the cursor in captures (payload: "true" or "false")
noctalia msg plugin mattestanka/screen-toolkit:service all setCursorHidden true
```
...
| `recordFullscreen` | — | Record a full monitor as MP4 (default) |
| `recordFullscreenGif` | — | Record a full monitor as GIF |
